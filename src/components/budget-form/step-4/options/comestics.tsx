          ...
          <div className="grid grid-cols-1 place-items-center gap-6">
            ...
            <div className="flex items-center gap-4">
              {modelColor.map(({ src, color, label }, index) => (
                <button
                  key={index}
                  className={`w-8 h-8 rounded-full border-4 shadow-lg p-1 border-neutral-200 cursor-pointer transition-all ${selected.modelLabel === label && "scale-125"}`}
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            {modelQtd.map(({ label, value }, index) => (
              <label
                ...
                className={`p-5 flex flex-col gap-2 items-center justify-center cursor-pointer rounded-lg cursor-pointerhover:translate-y-1 hover:shadow-lg hover:shadow-blue-300 ease transition-all ${selected.unitQtd === value ? "bg-blue-600 text-white" : "bg-white border text-neutral-800"}`}
              >
                <input
                  type="radio"
                  id={"capsule" + value}
